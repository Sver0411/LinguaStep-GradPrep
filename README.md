...
项目当前版本为 **v0.3.5**。无需注册账号；内置内容和学习记录保存在当前浏览器，断网时仍可继续核心学习。AI 是按需增强能力，没有配置 AI 也不影响其他功能。
...
- 生成日英对应词卡，并固定追加到现有词库；新批次不会替换旧批次。
- 生成日语语法、英语语法和日英语义对比，并追加到现有语法库。
- 单词页可一键“AI 新增 10 组并学习”，语法页可按当前语言模式现场新增内容。
- 生成结果与完整内置/AI 内容库双重去重；重复或不合格项会跳过并继续生成补足。
- 基于指定已学内容生成四选一练习题。
- 为错题生成六字段中文解释，并按题目、答案、Prompt 和模型缓存。
- 词汇与语法按批次事务追加保存；练习题可先预览，支持撤销最近批量新增和删除 AI 内容。
...
- AI 设置：总开关、接入方式、连接测试、默认生成参数、追加保存说明、质量复核、重试和每日软限制。
...
公开站点默认选择 BYOK，输入自己的 DeepSeek Key 后点击“保存”和“测试连接”即可；无需代理访问令牌。
