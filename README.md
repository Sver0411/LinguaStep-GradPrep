...
LinguaStep 是一个中文界面的个人日语与英语学习网页应用，面向日语 JLPT N3/N2/N1、英语高中基础→四级/六级/TOEIC的长期学习目标。它把日英对应词卡、语法、复习计划、测试、错题、收藏、统计和可选的 DeepSeek AI 集成在同一个学习闭环中。

项目当前版本为 **v0.5.1**。无需注册账号；内置内容和学习记录保存在当前浏览器，断网时仍可继续核心学习。AI 是按需增强能力，没有配置 AI 也不影响其他功能。
...
- 今日测试可一键开始，错题、到期、收藏和日英语法对比专项也提供快速预设；高级参数默认收起。
- 支持日语、英语、日英混合三种四选一测试模式。
- 同一轮先按词条/语法点去重，每项最多出现一次；中译日、中译英、日译英、英译日、日译中和英译中会分散到不同词条。
- 不同轮次会重新打散内容和题型；若可用的不同内容不足，宁可减少本轮题数，也不会用同一个词的多个翻译方向凑数。
...
AI 内容页位于“工具”分组，是可选的高级内容管理入口；日常新增可直接在单词和语法页完成。它提供：

- 生成日英对应词卡，并固定追加到现有词库；新批次不会替换旧批次。
- 生成日语语法、英语语法和日英语义对比，并追加到现有语法库。
- 单词页可一键“AI 新增 10 组并学习”；语法页默认一次新增 10 条，可在设置中调整为 10–30 条。
...
5. 需要扩充内容或更详细错因时，再使用 DeepSeek AI。
...
- 18 个自动化测试文件，共 **115 项测试通过**。
- 测试覆盖 6000 组词/50 个语法/137 组对比的完整性、日英对比混合测试、复习算法、每日计划、日期边界、三种学习模式、搜索筛选、测试判定、错题生命周期、统计聚合、设置、重置和 v1/v2→v3 数据迁移。
...
| v0.5.1 | 已完成 | 统一日语动词词条：普通动词保留词典原形，60 个「する」动词改为サ变词干收录 |
