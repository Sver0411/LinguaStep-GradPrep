...
function noun(spec: CoreSpec, index: number): WordPair {
  ...
  return {
    ...
    note: `「${japanese}」与 ${english} 都可表达“${meaningZh}”，实际使用时要留意固定搭配和语境。`,
...
function verb(spec: VerbSpec, index: number): WordPair {
  ...
  return {
    ...
    note: `日语「${japanese}」和英语 ${english} 的核心含义都是“${meaningZh}”，但宾语和介词搭配可能不同。`,
    highFrequency: rank === "高频",
    frequency: rank,
    source: "curated",
  };
...
function adjective(spec: CoreSpec, index: number): WordPair {
  ...
  return {
    ...
    note: `「${japanese}」与 ${english} 都用于描述“${meaningZh}”的性质，注意日语接名词时的词尾形式。`,
    highFrequency: rank === "高频",
    frequency: rank,
    source: "curated",
  };
...
function adverb(spec: AdverbSpec, index: number): WordPair {
  ...
  return {
    ...
    note: `「${japanese}」与 ${english} 都能表达“${meaningZh}”，但句中位置和标点习惯不同。`,
    highFrequency: rank === "高频",
    frequency: rank,
    source: "curated",
  };
