...
项目当前版本为 **v0.5.1**。无需注册账号；内置内容和学习记录保存在当前浏览器，断网时仍可继续核心学习。当前前端已暂停并移除 AI 入口，保留相关底层代码和既有内容数据，后续需要时可以重新启用。
...
- **内容可直接学习**：内置 6000 组日英对应词、50 个独立语法点和 137 组日英语法对比。
...
- 内置 **6000 组**日英对应词：保留原有 300 组人工精编内容，并基于 JMdict/EDRDG 与 ECDICT 扩展常用及进阶词汇。
...
- 18 个自动化测试文件，共 **126 项测试通过**。
- 测试覆盖 6000 组词/50 个语法/137 组对比/2160 道真题式练习的完整性、每种语言每个难度和题型 120 题的分布、真题语言与难度筛选、复习算法、每日计划、日期边界、三种学习模式、搜索筛选、测试判定、错题生命周期、统计聚合、设置、重置和 v1/v2→v3 数据迁移。
