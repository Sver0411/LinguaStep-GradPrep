  ...
  return (
    <div className="page-stack ai-page">
      <PageHeader
        ...
        description="AI 请求经同源服务端代理发送；返回内容必须经过结构、重复与语言校验，合格后才可进入本地学习库。"
      ...
      <section className="ai-overview-grid">
        <article className="card ai-generator-card">
          ...
          {tab !== "history" && (
            <div className="ai-generate-actions">
              <div><strong>{settings.defaultQuality === "quality" ? "Quality · deepseek-v4-pro" : "Fast · deepseek-v4-flash"}</strong><span>{settings.autoSave && tab !== "quiz" ? "生成后自动保存" : "生成后先预览"}{settings.qualityReview ? " · 启用质量复核" : ""}</span></div>
              {busy ? <Button variant="secondary" onClick={cancel}><X size={17} />取消生成</Button> : <Button disabled={configurationUnavailable || busyOperation !== null} onClick={() => void generateCurrent()}><Sparkles size={18} />开始生成</Button>}
            </div>
          )}
        ...
        <aside className="ai-usage-column">
          ...
          <article className="card ai-privacy-card"><BrainCircuit size={24} /><h3>发送什么？</h3><p>生成词汇和语法只发送表单参数与去重摘要；出题只发送最多 100 条相关学习内容摘要；密钥不写入请求正文。</p><Link href="/settings#deepseek-ai">查看连接与密钥设置</Link></article>
        </aside>
      </section>
      ...
      {result && result.kind === tab && (
        <section className="ai-result-section">
          <div className="section-title-row"><div><span className="section-kicker">VALIDATED RESULT</span><h2>{result.payload.generation.status === "partial" ? "部分内容通过校验" : "生成与校验已完成"}</h2><p>{result.payload.generation.acceptedCount} 项通过 · {result.payload.generation.rejectedCount} 项拒绝 · {result.payload.generation.model}</p></div><div className="page-actions">{!result.saved && result.kind !== "quiz" && <Button onClick={() => void saveTransient()}><Save size={17} />保存到学习库</Button>}{result.saved && result.kind !== "quiz" && <Button variant="secondary" onClick={() => void undoLastSave()}><Undo2 size={17} />撤销保存</Button>}</div></div>
          ...
          {result.payload.words && <div className="ai-preview-grid">{result.payload.words.map((word) => <article className="card ai-preview-card" key={word.id}><div className="tag-row"><span>{word.japanese.difficulty}</span><span>{word.english.difficulty}</span><span>{word.frequency}</span></div><h3>{word.meaningZh}</h3><p><b>日</b> {word.japanese.term} <small>{word.japanese.reading}</small></p><p><b>英</b> {word.english.term} <small>{word.english.phonetic}</small></p><p>{word.note}</p><div className="ai-card-actions"><button className="text-button" onClick={() => void reportContent("word", word.id, result.payload.generation.id)}>反馈问题</button>{result.saved && <button className="text-button danger-text" onClick={() => void removeAIContent([`word:${word.id}`])}><Trash2 size={14} />删除</button>}</div></article>)}</div>}
          {result.payload.grammar && <div className="ai-preview-grid">{result.payload.grammar.map((point) => <article className="card ai-preview-card" key={point.id}><div className="tag-row"><span>{point.language === "japanese" ? "日语" : "英语"}</span><span>{point.level}</span></div><h3>{point.title}</h3><p>{point.explanation}</p><strong>{point.structure}</strong><div className="ai-card-actions"><button className="text-button" onClick={() => void reportContent("grammar", point.id, result.payload.generation.id)}>反馈问题</button>{result.saved && <button className="text-button danger-text" onClick={() => void removeAIContent([`grammar:${point.id}`])}><Trash2 size={14} />删除</button>}</div></article>)}</div>}
          {result.payload.comparisons && <div className="ai-preview-grid">{result.payload.comparisons.map((item) => <article className="card ai-preview-card" key={item.id}><div className="tag-row"><span>日英对比</span><span>{item.level}</span></div><h3>{item.semantic}</h3><p><b>日</b> {item.japanese}</p><p><b>英</b> {item.english}</p><p>{item.difference}</p><div className="ai-card-actions"><button className="text-button" onClick={() => void reportContent("comparison", item.id, result.payload.generation.id)}>反馈问题</button>{result.saved && <button className="text-button danger-text" onClick={() => void removeAIContent([`comparison:${item.id}`])}><Trash2 size={14} />删除</button>}</div></article>)}</div>}
