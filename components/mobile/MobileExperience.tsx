...
function MobileWordLibrary() {
  const { allWords, snapshot } = useLearning();
  const [query, setQuery] = useState("");
  const normalized = query.trim().toLocaleLowerCase("zh-CN");
  const words = useMemo(() => allWords.filter((word) => !normalized || [word.japanese.term, word.english.term, word.meaningZh].some((value) => value.toLocaleLowerCase("zh-CN").includes(normalized))).slice(0, 36), [allWords, normalized]);
  const progress = useMemo(() => new Map(snapshot.wordProgress.map((item) => [item.wordId, item])), [snapshot.wordProgress]);
  return <main className="m3-page"><MobileSubHeader detail="WORD LIBRARY" onBack={() => navigateTo("/")} title="单词库" /><label className="m3-search"><Search size={17} /><input autoFocus onChange={(event) => setQuery(event.target.value)} placeholder="搜索日语、英语或中文" value={query} /></label><div className="m3-section-heading"><h2>{normalized ? "搜索结果" : "全部词汇"}</h2><span>{words.length} 条</span></div><section className="m3-word-list">{words.map((word) => { const state = getWordModeState(progress.get(word.id), "japanese"); return <button key={word.id} onClick={() => navigateTo(mobileHref("/words", { mobile: "study", word: word.id, mode: "japanese" }))} type="button"><span>{word.japanese.term}</span><div><b>{word.meaningZh}</b><small>{word.english.term} · {state ? state.status === "mastered" ? "已掌握" : "学习中" : "未学习"}</small></div><ChevronRight size={18} /></button>; })}</section>{words.length === 0 && <p className="m3-empty">没有找到匹配的词汇。</p>}</main>;
}
