...
日语目标：${input.japaneseLevel}；英语目标：${input.englishLevel}；频率：${input.frequency}；用途：${input.purpose}。
...
export const qualityReviewPrompt: PromptTemplate<{ content: unknown; kind: string }> = {
  ...
  maxTokens: 1500,
  system: `${SHARED_RULES}
任务：审查语言学习内容质量。输出严格为 {"status":"pass|needsRepair|reject","issues":[],"repairSuggestions":[]}。
只审查最终内容，不复述或输出内部推理。`,
